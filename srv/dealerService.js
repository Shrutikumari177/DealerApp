...
module.exports = async (srv) => {
    ...
    //         if (SerialNo) {
    //             const existingBox = await externalService.run(
    //                 SELECT.one.from('MaterialBox').where({ SerialNo: SerialNo })
    //             );
    ...
    //             if (existingBox.RetailerId) {
    //                 return `MaterialBox ${SerialNo} is already mapped to Retailer ${existingBox.RetailerId}. Skipping update.`;
    //             }
    ...
    //             if (existingIC.RetailerId) {
    //                 return `InnerContainer ${IC_ICID} is already mapped to Retailer ${existingIC.RetailerId}. Skipping update.`;
    //             }
    ...
    //             return `InnerContainer ${IC_ICID} has been successfully mapped to Retailer ${RetailerId}.`;
    //         }
    ...
    //             if (!existingOC) {
    //                 return req.reject(404, `OuterContainer with OCID: ${OC_OCID} not found.`);
    //             }
    ...
    //             const innerContainers = await externalService.run(
    //                 SELECT.from('InnerContainer').where({ OC_OCID: OC_OCID, RetailerId: { '!=': RetailerId } })
    //             );
